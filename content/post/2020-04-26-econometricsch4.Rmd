---
title: '計量経済学講義：単回帰'
author: 嶋恵一
date: '2020-04-26'
slug: EconometricsCh4
categories:
  - Lecture
tags:
  - Econometrics
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

この遠隔講義では、講義ノート第4章「単回帰」に進みます。以下は第4章と同じ内容をwebに掲載したものです。ただし、練習問題を追加しております。PDFを読む方が勉強しやすいと思います。

講義ノート第4章を自習しながら、分からないところをノートにまとめて下さい。後ほど、こちらから連絡致しますので、分からないところ中心に解説することを予定しております。最後に、練習問題を解いて、期限を決めて提出してもらおうと考えております。

それでは、講義ノート第4章を学習しましょう。


# 最小二乗法による推定値

真の回帰方程式を
$$
y_i =\alpha +\beta x_i +u_i
\tag{1}
$$
とする。ただし、$y_i$、$x_i$は観測される変数であり、標本数は$n$とする。誤差項は未知であり、$u_{i}\sim N\left(  0,\sigma^2\right)$を仮定する。
$\alpha,\beta$は未知係数であり、最小二乗法（OLS）による推定値を$\hat{\alpha},\hat{\beta}$と表わす。誤差項の推定値を残差と呼び、$\hat u_i=y_i-\hat{\alpha}-\hat{\beta}x_i$により表わす。このとき、残差二乗和$J=\sum\hat{u}_i^2$の最小化より、$\hat{\alpha},\hat{\beta}$に関する以下の正規方程式が導かれる。
$$
\left\{
\begin{array}{c}
\sum y_{i}-n\hat{\alpha}-\hat{\beta}\sum x_{i}=0,\\
\sum y_{i}x_{i}-\hat{\alpha}\sum x_{i}-\hat{\beta}\sum x_{i}^{2}=0
\end{array}
\right.
\tag{2}
$$
(2)式の一段目より、
$$
\hat{\alpha}=\overline{y}-\hat{\beta}\overline{x}
\tag{3}
$$
が得られる。ただし、$\overline{x}$、$\overline{y}$は平均を表す。(3)式を(2)式の二段目に代入すれば、次式が得られる。
$$
\begin{aligned}
\sum y_{i}x_{i}-\left(  \overline{y}-\hat{\beta}\overline{x}\right)
n\overline{x}-\hat{\beta}\sum x_{i}^{2}  &  =0,\\
\sum y_{i}x_{i}-n\overline{y}\overline{x}-\hat{\beta}\left(  \sum
x_{i}^{2}-n\overline{x}^{2}\right)   &  =0 
\end{aligned}
\tag{4}
$$

ここで、$S_{yx}=\sum y_{i}x_{i}-n\overline{y}\overline{x}$、$S_{xx}=\sum x_i^2-n\overline{x}^2$と置く。以上を用いて(4)式を書き直せば、次が得られる。
$$
S_{yx}=S_{xx}\hat{\beta}
$$
これを解けば、$\hat{\beta}$は次のように求められる。
$$
\hat{\beta}=\frac{S_{yx}}{S_{xx}} \tag{5}
$$
また、$\hat{\beta}$の推定値を(3)式に代入すれば、
$$
\hat{\alpha}=\overline{y}-\frac{S_{yx}}{S_{xx}}\overline{x} \tag{6}
$$
が求まる。


# 推定値の分散


最初に、$\hat{\beta}$の期待値と分散を求める。(5)式より、$\hat{\beta}$の右辺の分子$S_{yx}$は
$$
S_{yx}=\left( \sum y_i x_i -n\overline{y}\overline{x}\right)
=\sum\left(y_{i}-\overline{y}\right)\left(x_i-\overline{x}\right)
\tag{7}
$$
と書ける。真の回帰方程式(1)について、その平均は以下を満たす：
$$
\overline{y}=\alpha+\beta\overline{x}+\overline{u} \tag{s8}
$$
ただし、$\overline{u}=\sum u_i/n$は未知である。真の回帰方程式(1)とその平均である(8)式とを(7)式に代入すれば、右辺は次のように書き直せる。
$$
\begin{aligned}
& \sum\left(y_i-\overline{y}\right) \left(  x_i-\overline{x}\right)\\
& =\sum\left[ \alpha+\beta x_i+u_i-\left( \alpha+\beta\overline{x} +\overline{u}\right) \right] \left(x_i-\overline{x}\right)\\
& =\sum\left[ \beta\left( x_i-\overline{x}\right) +u_i-\overline{u} \right]  \left( x_i-\overline{x}\right)\\
& =\beta\sum\left(x_i-\overline{x}\right) ^2+\sum u_i\left(
x_i-\overline{x}\right) -\overline{u}\sum\left( x_i-\overline{x}\right)\\
& =\beta S_{xx}+\sum u_i\left(  x_i-\overline{x}\right)  \end{aligned}
\tag{9}
$$
(9)式を用いて(5)式の分子を書き換えれば、$\hat{\beta}$に関する次の表現が得られる。
$$
\hat{\beta}=\beta+\frac{\sum u_i\left(x_i-\overline{x}\right)}{S_{xx}} 
\tag{10}
$$

OLSでは真の誤差項と説明変数とが独立（無相関）であること、すなわち$E\left[  u_ix_i\right] =0$を仮定する。かつ、異なる時点$i\neq j$の誤差項も独立であり、$E\left[  u_i u_j \right]=0$を仮定する。このとき、
$$
E\left(  \hat{\beta}\right)  =\beta
$$
が成り立つ。OLSにより求められた$\hat{\beta}$の期待値が何の偏りもなく真の$\beta$に一致する性質を「不偏性」と呼ぶ。

一方、分散は(10)式を用いることにより、次式で表すことができる。
$$
E\left( \hat{\beta}-\beta \right) ^2 
= E\left[ \frac{\sum u_i\left(x_i-\overline{x}\right)}{S_{xx}}
\right]^2 
\tag{11}
$$
ここで、(11)式の分子を展開すれば、次の表現が得られる。
$$
\begin{aligned}
&  E\left[  \sum u_{i}\left(  x_{i}-\overline{x}\right)  \right]^2\\
&  =E\left[  \sum_{i}\sum_{j}u_{i}u_{j}\left(  x_{i}-\overline{x}\right)
\left(  x_{j}-\overline{x}\right)  \right]  
\end{aligned}
\tag{12}
$$
OLSの仮定より、$E\left(u_i u_j\right)=0$、
$E\left(u_j^2\right)=\sigma^2$を用いれば、以上は
$$
\begin{aligned}
E\left[  \sum_{i}\sum_{j}u_{i}u_{j}\left(  x_{i}-\overline{x}\right) \left(
x_{j}-\overline{x}\right)  \right]   &  =\sum_{i}\sum_{j}\left(
x_{i}-\overline{x}\right)  \left(  x_{j}-\overline{x}\right)  E\left(
u_{i}u_{j}\right) \\
&  =\sum_{j}\left(  x_{j}-\overline{x}\right)  \left(  x_{j}-\overline
{x}\right)  E\left(  u_{j}^{2}\right) \\
& =S_{xx}\sigma^{2} 
\end{aligned}
\tag{13}
$$
と書き直せる。よって、(11)-(13)式により、$\hat{\beta}$の分散は次のように求められる。
$$
E\left(  \hat{\beta}-\beta\right)  ^{2}=\frac{S_{xx}\sigma^{2}}{S_{xx}^{2}}=\frac{\sigma^{2}}{S_{xx}}
$$

次に、$\hat{\alpha}$の期待値と分散を求める。(8)式の代入により、(3)式は
$$
\begin{aligned}
\hat{\alpha}  &  =\alpha+\beta\overline{x}+\overline{u}-\hat{\beta
}\overline{x}\nonumber\\
&  =\alpha-\left(  \hat{\beta}-\beta\right)  \overline{x}+\overline{u}
\end{aligned}
\tag{14}
$$
と書ける。従って、$\hat{\alpha}$の期待値は次のように表現できる。 
$$
\begin{aligned}
E\left(  \hat{\alpha}\right)   &  =E\left[  \alpha-\left(  \hat{\beta}-\beta\right)  \overline{x}+\overline{u}\right] \\
&  =\alpha-\overline{x}E\left(  \hat{\beta}-\beta\right)  +E\left(
\overline{u}\right)
\end{aligned}
$$
OLSの仮定より、$E\left[\overline{u}\right] =\sum E\left( u_i\right)=0$が成り立つ。更に$E\left( \hat{\beta}\right)=\beta$より、
$$
E\left(  \hat{\alpha}\right)  =\alpha
$$
が得られる。$\hat{\beta}$と同じく、$\hat{\alpha}$は不偏性を持つ。

一方、分散は(14)式より、次のように表すことができる。
$$
E\left(  \hat{\alpha}-\alpha\right)  ^{2}=E\left[  -\left(  \hat
{\beta}-\beta\right)  \overline{x}+\frac{1}{n}\sum u_{i}\right]  ^{2}
\tag{15}
$$
ここで、OLSの仮定より$E\left(u_i u_j \right)=0$、$E\left(u_j^2\right)=\sigma^2$を用いれば、(15)式の右辺を次のように書き直すことができる。
$$
\begin{aligned}
&  E\left[  \left(  \hat{\beta}-\beta\right)  ^{2}\overline{x}^{2}+\frac{1}{n^{2}}\sum u_{i}\sum u_{j}-\frac{2}{n}\left(  \hat{\beta}-\beta\right)  \overline{x}\sum u_{i}\right] \nonumber\\
&  =\overline{x}^{2}E\left(  \hat{\beta}-\beta\right)  ^{2}+\frac{1}{n}\sigma^{2}-\frac{2\overline{x}}{n}E\left[  \left(  \hat{\beta}-\beta\right)  \sum_{i}u_{i}\right]
\end{aligned}
\tag{16}
$$
(16)式の第3項を整理すれば、
$$
\begin{aligned}
E\left[  \left(  \hat{\beta}-\beta\right)  \sum_{i}u_{i}\right]   &
=E\left[  \frac{\sum u_{j}\left(  x_{j}-\overline{x}\right)  }{S_{xx}}\sum
_{i}u_{i}\right]\\
&  =\frac{1}{S_{xx}}E\left[  \sum u_{j}\left(  x_{j}-\overline{x}\right)
\sum_{i}u_{i}\right]\\
&  =\frac{1}{S_{xx}}E\left[  \sum u_{j}^{2}\left(  x_{j}-\overline{x}\right)
\right]\\
&  =\frac{\sigma^{2}}{S_{xx}}\sum\left(  x_{j}-\overline{x}\right)  =0
\end{aligned}
\tag{17}
$$
となる。よって、(15)-(17)式により、$\hat{\alpha}$の分散は次のように求められる。
$$
\begin{aligned}
E\left(  \hat{\alpha}-\alpha\right)  ^{2}  &  =\overline{x}^{2}\frac{\sigma^{2}}{S_{xx}}+\frac{1}{n}\sigma^{2}\\
&  =\left(  \frac{\overline{x}^{2}}{S_{xx}}+\frac{1}{n}\right)  \sigma^{2}\\
&  =\frac{n\overline{x}^{2}+S_{xx}}{nS_{xx}}\sigma^{2}\\
&  =\frac{\sum x_{i}^{2}}{nS_{xx}}\sigma^{2}\end{aligned}
$$


# 決定係数


回帰分析には、推定される回帰式のパフォーマンスを評価するための指標があり、それを決定係数と呼ぶ。具体的には次のようなアイデアに基づく。

$n$個のデータ$(x_i,y_i)$に対して、OLSにより$\hat{y}_i=\hat{\alpha}+\hat{\beta}x_i$を求める。残差を$\hat{u}_i=y_i-\hat{y}_i$と定義する。正規方程式の解より、$\sum \hat{u}_{i}=0$、$\overline{y}=\hat{\alpha}+\hat{\beta}\overline{x}$が成立する。すなわち残差の合計はゼロであり、かつ回帰直線は$X$と$Y$のデータの平均を必ず通過する。また、真の$y_i$と回帰式により推定された$\hat{y}_i$との間には、次の関係が成り立つ。
$$
y_{i}=\hat{\alpha}+\hat{\beta}x_{i}+\hat{u}_{i} \tag{18}
$$

ここで、$y_i$についてその平均を$\overline{y}$と表し、$y_{i}$の平均$\overline{y}$からの偏差を用いて、$y_{i}$のばらつきを次のように定義する。
$$
\sum\left(  y_{i}-\overline{y}\right)  ^{2} \tag{19}
$$
(19)式で表される$y_i$の平均$\overline{y}$からの偏差の二乗の総和を「平均周りの変動和」と呼ぶ。(18)式の両辺から$\overline{y}$を引き整理すれば次が得られる：
$$
\begin{aligned}
y_{i}-\overline{y}  &  =\hat{\alpha}+\hat{\beta}x_{i}+\hat{u}_{i}-\hat{\alpha}-\hat{\beta}\overline{x}\\
&  =\hat{\beta}\left(  x_{i}-\overline{x}\right)  +\hat{u}_{i}
\end{aligned}
\tag{20}
$$
(20)式の両辺をそれぞれ二乗すれば、次の関係が得られる。
$$
\begin{aligned}
\sum\left(  y_{i}-\overline{y}\right)  ^{2}  &  =\sum\left(  \hat{\beta
}\left(  x_{i}-\overline{x}\right)  +\hat{u}_{i}\right)  ^{2}\\
&  =\sum\hat{\beta}^{2}\left(  x_{i}-\overline{x}\right)  ^{2}+\sum\hat{u}_{i}^{2}+2\hat{\beta}\sum\left(  x_{i}-\overline
{x}\right)  \hat{u}_{i} 
\end{aligned}
\tag{21}
$$
ただし、正規方程式より$\sum x_i\hat{u}_{i}=0$かつ$\sum\hat{u}_{i}=0$より、(21)式は以下のように書き直せる。
$$
\sum\left(  y_{i}-\overline{y}\right)  ^{2}=\hat{\beta}^{2}\sum\left(
x_{i}-\overline{x}\right)  ^{2}+\sum\hat{u}_{i}^{2}
\tag{22}
$$
(22)式の左辺は前述の$y_i$の平均周りの変動和である。他方、右辺は$x_{i}$の平均周りの変動和$\sum\left(x_i-\overline{x}\right)^2$の$\hat{\beta}^2$倍と残差の変動和$\sum\hat{u}_i^2$との合計で表される。すなわち、$y_i$の変動和は「単回帰によって説明される変動部分」と回帰式では説明できない「残差の変動部分」とに切り分けることができる。

ここで、式の両辺を$\sum\left(y_i-\overline{y}\right)^2$で割ると、次の関係式が得られる。
$$
\frac{\hat{\beta}^{2}\sum\left(  x_{i}-\overline{x}\right)  ^{2}}{\sum\left(  y_{i}-\overline{y}\right)  ^{2}}+\frac{\sum\hat{u}_{i}^{2}}{\sum\left(  y_{i}-\overline{y}\right)  ^{2}}=1
$$
さらに、
$$
\frac{\hat{\beta}^{2}\sum\left(  x_{i}-\overline{x}\right)  ^{2}}{\sum\left(  y_{i}-\overline{y}\right)  ^{2}}=w
$$
と定義する。$w$は単回帰の説明変数$x_i$の変動和によって$y_i$の変動和を説明できる割合を表す。これより、上述の関係式は次のように書き直せる。
$$
w+\frac{\sum\hat{u}_{i}^{2}}{\sum\left(  y_{i}-\overline{y}\right)  ^{2}}=1
$$
ここでは$w\geq0$、$\sum\hat{u}_i^{2}\geq 0$であり、単回帰の説明変数の変動和によって$y_i$の変動和の多くを説明できるほど$w$は1に近づく。反対に、単回帰が$y$の変動和を説明できないほど残差変動和の占める割合は大きくなり、$w$は0に近づく。被説明変数である$y$の変動和に着目し、それを回帰式で説明できる変動部分と説明できない残差による変動部分とに分け、回帰式の貢献比率を表したものが決定係数のアイデアである。

より正確な定義は以下の通りである。 
$$
\begin{aligned}
y_{i}-\overline{y}  &  =\left(  y_{i}-\hat{y}_{i}\right)  +\left(
\hat{y}_{i}-\overline{y}\right) \\
&  =\left(  \hat{y}_{i}-\overline{y}\right)  +\hat{u}_{i}
\end{aligned}
$$
この両辺をそれぞれ二乗すれば、次の関係が得られる。
$$
\begin{aligned}
\sum\left(  y_{i}-\overline{y}\right)  ^{2}  &  =\sum\left(  \left(
\hat{y}_{i}-\overline{y}\right)  +\hat{u}_{i}\right)  ^{2}\\
&  =\sum\left(  \hat{y}_{i}-\overline{y}\right)  ^{2}+\sum\hat{u}_{i}^{2}+2\sum\left(  \hat{y}_{i}-\overline{y}\right)  \hat{u}_{i}
\end{aligned}
$$
ただし、正規方程式より、
$$
\sum\hat{y}_{i}\hat{u}_{i}=\hat{\alpha}\sum\hat{u}_{i}+\hat{\beta}\sum x_{i}\hat{u}_{i}=0
$$
であるため、次が導ける。
$$
\sum\left(  y_{i}-\overline{y}\right)  ^{2}=\sum\left(  \hat{y}_{i}-\overline{y}\right)  ^{2}+\sum\hat{u}_{i}^{2}
$$
以上の両辺を$\sum\left(  y_{i}-\overline{y}\right)^{2}$で割ると次の関係が得られる：
$$
1=\frac{\sum\left(  \hat{y}_{i}-\overline{y}\right)  ^{2}}{\sum\left(
y_{i}-\overline{y}\right)  ^{2}}+\frac{\sum\hat{u}_{i}^{2}}{\sum\left(
y_{i}-\overline{y}\right)  ^{2}}
$$

ここで、
$$
\frac{\sum\left(  \hat{y}_{i}-\overline{y}\right)  ^{2}}{\sum\left(
y_{i}-\overline{y}\right)  ^{2}}=R^{2}
$$
と定義する。これより、以上の関係式は次のように書き直せる。
$$
R^2=1-\frac{\sum\hat{u}_i^2}{\sum\left( y_i-\overline{y}\right)^2}
$$
これが決定係数の定義であり、通常$R^2$で表記される。決定係数は、推定された回帰式が被説明変数の変動をどれだけ説明できるかを表す比重である。


# 練習問題

以下の問題の解答を作成し提出せよ。

**問1**　二つの変数$x_i$、$y_i$がペアで観察され、$i=1…n$の標本が存在する。両変数の間に回帰式$y_i=\alpha+\beta x_i+u_i$を当てはめ、最小二乗法によるパラメータ推定$\hat{\alpha}$、$\hat{\beta}$を考える。誤差項$u_i$は平均0、定数の分散に従う確率変数と仮定する。推定される残差を$\hat{u}_i=y_i-\hat{\alpha}-\hat{\beta} x_i$と定義する。$x_i$、$y_i$の平均を$\overline{x}=\frac{1}{n} \sum_{i=1}^n x_i$、$\overline{y}=\frac{1}{n} \sum_{i=1}^n y_i$と表す。

**(i)**	正規方程式より、$\sum_{i=1}^n \hat{u}_i=0$、かつ、$\sum_{i=1}^n x_i \hat{u}_i=0$であることを示せ。

**(ii)**	正規方程式を解き、$\hat{\alpha}_i=\overline{y}_i-\hat{\beta} \overline{x}_i$となることを示せ。

**(iii)**	同じく、$\hat{\beta}=\sum_{i=1}^n \left(  x_{i}-\overline{x}\right) \left(  y_{i}-\overline{y}\right)/ \sum_{i=1}^n \left(  x_{i}-\overline{x}\right)^2$となることを示せ。

**問2**　両変数の間に回帰式$y_i=\alpha+\beta x_i+u_i$を当てはめ、最小二乗法による推定結果が$\hat{\alpha}$、$\hat{\beta}$として求められたとする。以下を説明せよ。

**(i)**	説明変数$x_i$がすべて半分（1/2倍）になったとき、$\hat{\alpha}$、$\hat{\beta}$はどのように変わるか。

**(ii)**	説明変数$x_i$がすべて$x_i-2$になったとき、$\hat{\alpha}$、$\hat{\beta}$はどのように変わるか。
